...
int main(int ac, char **av)
{
	...
	int count = 0;
	criminals = getCriminals("criminals.txt", &count);
	...
	sortCriminals(criminals, count);
	info = getInfo(av[1]);
	potentialCriminals = findPotentialCriminals(criminals, count, info);
...
struct s_criminal **getCriminals(char *filename, int *count)
{
	FILE				*f;
	struct s_criminal	**criminals;
	size_t 				linecap = 0;
